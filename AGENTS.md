...
1. **每次代码修改完成后，必须向用户提供可访问的预览链接**：
   - 链接必须从环境变量 `COZE_PROJECT_DOMAIN_DEFAULT` 实时读取（`echo $COZE_PROJECT_DOMAIN_DEFAULT`），**严禁**凭记忆或使用示例域名
   - 当前开发环境预览地址：`https://a5d915e5-383b-4c0c-9771-a7f4ce33db01.dev.coze.site`（以实时读取为准）
   - 主仓预览常驻 5000 端口，代码保存后 HMR 自动生效，无需重启
2. **部署必须经过用户确认**：用户预览确认没问题并明确说"部署"后，才允许执行部署动作（`coze build` / 生产发布）；在此之前只交付预览链接
3. 交付预览链接时附上本次修改的功能入口路径，方便用户直接点开对应页面验收
